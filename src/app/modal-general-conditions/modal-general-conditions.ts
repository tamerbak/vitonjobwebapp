...
@Component({
  selector: '[modal-general-conditions]',
  template: require('./modal-general-conditions.html'),
  directives: [ModalOptions]
})

export class ModalGeneralConditions{
  private gConditions:{
    title:string,
  ...
  private dataValidation: boolean = false;
  private modalParams: any = {type: '', message: ''};
  ...
  close(): void {
    jQuery('#modal-general-conditions').modal('hide');
  }

  logOut() {
    Home.logOut(this.currentUser);
  }
}
