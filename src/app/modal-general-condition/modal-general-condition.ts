...
@Component({
  ...
  template: require('./modal-general-condition.html'),
  providers: [UserService]
})

export class ModalGeneralCondition{
  currentUser: any;
  projectTarget: string;

  refusalMsg: string;
  hasRefused: boolean = false;
  @Output()
  onRefuse = new EventEmitter<boolean>();
  ...
  connect() {
    this.userService.updateGCStatus("Oui", "Non", this.currentUser.id, this.projectTarget).then((response) => {
      this.hasRefused = false;
      this.onRefuse.emit(this.hasRefused);
      this.close();
    });
  }
  ...
  logOut(contactMe) {
    ...
    this.userService.updateGCStatus("Non", isContact, this.currentUser.id, this.projectTarget).then((response) => {
          this.hasRefused = true;
          this.onRefuse.emit(this.hasRefused);
          this.close();
    })
  ...
  close(): void {
    jQuery('#modal-general-condition').modal('hide');
  }
