  ...
  editOffer() {
    ...
    if (this.obj != "detail") {
      ...
      this.offersService.setOfferInRemote(this.offer, this.projectTarget).then((data: any)=> {
        ...
        let createdOffer = JSON.parse(data._body);
        if (this.projectTarget == 'employer') {
          this.currentUser.employer.entreprises[0].offers.push(createdOffer);
        } else {
          this.currentUser.jobyer.offers.push(createdOffer);
        }
        this.sharedService.setCurrentUser(this.currentUser);
        Messenger().post({
